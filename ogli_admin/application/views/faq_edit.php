					...
					// echo "<pre>"; print_r($editfaq); 
						...
						{ 
							foreach($editfaq as $key => $val)
							{
								...
								<div class="row">
									<div class="col-6">
										<form action="<?php echo base_url()?>index.php/Faq/edit_faq_data" method="post" data-parsley-validate class="form-horizontal form-label-left" enctype="multipart/form-data">
											<input type="hidden" name="editid" id="editid" value="<?php echo $val->MQa_id;?>">
													...
													<select class="form-control select2" name="pagename" id="pagename" style="width:100%;">                     
														...
														if($parentmenulist){
															...
															if(!empty($childmenulist)){ ?>
																...
																<option value="<?php echo $chmenu->MM_ID; ?>" <?php if(!empty($val->MM_ID)) {  if($val->MM_ID == $chmenu->MM_ID) echo 'selected'; } ?> ><?php echo $chmenu->MM_Name; ?></option>
																<?php } ?>
																</optgroup>
															<?php  }else{ ?>
																<option value="<?php echo $pmenu->MM_ID; ?>" <?php if(!empty($val->MM_ID)) {  if($val->MM_ID == $pmenu->MM_ID) echo 'selected'; } ?> ><?php echo $pmenu->MM_Name; ?></option>
															<?php  }                                
															}
														}
														?>
											...
											<div class="form-group mb-0 overflow-hidden mt-3">
												...
												<input name="MQa_title" class="form-control" type="text" value="<?php echo $val->MQa_title; ?>">                             
											</div>
											<div class="form-group mb-0 overflow-hidden mt-3">
												<label for="metakeyword">Answaer</label>
												<textarea class="form-control inputheight" rows="5" name="MQa_answer"><?php echo $val->MQa_answer; ?></textarea>
											</div>
											<div class="form-group mb-0 overflow-hidden mt-4">
												...
												<input type="checkbox" Value="1" name="status" id="status" class="custom-switch-input" <?php if(!empty($val->MQa_status)){ echo ($val->MQa_status =='1')?'checked':''; } ?> >
...
echo my_file1('web/custom/faq.js?v=191120.0',2);
?>
